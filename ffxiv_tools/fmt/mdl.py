...
def mdl(c):
    ...
    type7s = [ type7(c) for _ in range(header.type7_count) ]
    lods = [ lod(c) for _ in range(3) ]
    ...
    if sum(l.mesh_count for l in lods) + sum(l.mesh_count_2 for l in lods) + sum(l.mesh_count_3 for l in lods) + sum(l.mesh_count_4 for l in lods) != header.mesh_count:
        unknown_data = [ t.raw(c, 40) for _ in range(3) ]

    meshes = [ mesh(c) for _ in range(header.mesh_count) ]

    type1_string_offsets = [ t.uint32(c) for _ in range(header.type1_count) ]

    type3s = [ type3(c) for _ in range(header.type3_count) ]
    bones_links = [ bones_link(c) for _ in range(header.bones_links_count) ]
    faces = [ face(c) for _ in range(header.face_count) ]

    materials_string_offsets = [ t.uint32(c) for _ in range(header.material_count) ]
    bones_string_offsets = [ t.uint32(c) for _ in range(header.bone_count) ]

    type6s = [ type6(c) for _ in range(header.type6_count) ]
    type8s = [ type8(c) for _ in range(header.type8_count) ]
    type9s = [ type9(c) for _ in range(header.type9_count) ]
    type10s = [ type10(c) for _ in range(header.type10_count) ]

    bone_indexes_size = t.uint32(c)
    bone_indexes = [ t.uint16(c) for _ in range(bone_indexes_size // 2) ]

    offset_to_bounding_boxes = t.uint8(c)
    c.skip(offset_to_bounding_boxes)

    bounding_boxes = [ bounding_box(c) for _ in range(2 + 2 + header.bone_count + header.unknown9) ]
...
def type6(c):
    return nt('Type6',
        bone_indexes = [ t.uint16(c) for _ in range(64) ],
        unknown2     = t.uint32(c)
    )
