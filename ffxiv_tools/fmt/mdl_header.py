...
def mdl_header(c):
    ...
    type7s = t.array(c, type7, head.type7_count)
    lods = t.array(c, lod, 3)

    unknown_data = None
    if sum(l.mesh_count + l.mesh_count_2 + l.mesh_count_3 + l.mesh_count_4 for l in lods) != head.mesh_count:
        unknown_data = t.array(c, t.raw, 3, 40)

    meshes = t.array(c, mesh, head.mesh_count)

    type1_string_offsets = t.array(c, t.uint32, head.type1_count)

    type3s = t.array(c, type3, head.type3_count)
    bones_links = t.array(c, bones_link, head.bones_links_count)
    faces = t.array(c, face, head.face_count)

    materials_string_offsets = t.array(c, t.uint32, head.material_count)
    bones_string_offsets = t.array(c, t.uint32, head.bone_count)

    type6s = t.array(c, type6, head.type6_count)
    type8s = t.array(c, type8, head.type8_count)
    type9s = t.array(c, type9, head.type9_count)
    type10s = t.array(c, type10, head.type10_count)
    ...
    bounding_boxes = t.array(c, bounding_box, 2 + 2 + head.bone_count + head.unknown9)

    materials_names = names(c, strings_offset, materials_string_offsets)
    bones_names = names(c, strings_offset, bones_string_offsets)
...
@binr.struct
def names(c, strings_offset, offsets):
    rv = []
