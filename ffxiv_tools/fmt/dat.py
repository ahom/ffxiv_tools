...
def std_file(c, fh, file_offset):
    ...
    output = BytesIO()
    for block_header in [ std_file_block_header(c) for _ in range(block_count) ]:
...
def mdl_file(c, fh, file_offset):
    ...
    block_sizes = [ t.uint16(c) for _ in range(block_count) ]
...
def mdl_file_block_headers(c):
    ...
    rv = nt('MdlFileBlockHeaders',
        uncompressed_sizes = [ t.uint32(c) for _ in range(MDL_FILE_BLOCK_HEADERS_COUNT) ],
        sizes = [ t.uint32(c) for _ in range(MDL_FILE_BLOCK_HEADERS_COUNT) ],
        offsets = [ t.uint32(c) for _ in range(MDL_FILE_BLOCK_HEADERS_COUNT) ],
        block_id_starts = [ t.uint16(c) for _ in range(MDL_FILE_BLOCK_HEADERS_COUNT) ],
        block_counts = [ t.uint16(c) for _ in range(MDL_FILE_BLOCK_HEADERS_COUNT) ]
    )
...
def tex_file(c, fh, file_offset):
    ...
    mipmap_block_headers = [ tex_mipmap_block_header(c) for _ in range(mipmap_count) ]
